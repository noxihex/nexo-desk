...
@section('title', config('app.name') . ' - Editar Contrato')

@section('content_header')
    <p style="font-size: 1.2em;">
        Cadastros <i class="fas fa-angle-right" style="font-size: 0.7em;"></i> Contratos <i class="fas fa-angle-right" style="font-size: 0.7em;"></i> Editar
    </p>
@stop
...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Atualize as informações do contrato</h3>
        </div>

        <div class="card-body">
            {{-- O ID do formulário é importante para o script --}}
            <form id="form-contrato" action="{{ route('contratos.update', $contrato->id) }}" method="POST">
                @csrf
                @method('PUT')
                @include('cadastros.contratos._form', ['contrato' => $contrato])
