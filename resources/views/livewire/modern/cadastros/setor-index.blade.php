<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <x-modern.button :href="route('setores.create')" variant="filled" color="green" icon="plus">Novo setor</x-modern.button>
    </div>
    ...
    <x-modern.card class="overflow-hidden">
        <x-modern.table >
            ...
            <x-modern.table.rows>
                    ...
                    <x-modern.table.row :key="$record->id">
                        ...
                        <x-modern.table.cell>
                            <div class="flex flex-wrap gap-2">
                                <x-modern.button :href="route('setores.edit', $record)" variant="filled" color="amber" size="sm" icon="pencil-square" :aria-label="'Editar '.$record->nome">Editar</x-modern.button>
                ...
                @empty
                    <x-modern.table.row>
                        <x-modern.table.cell colspan="2" class="py-10 text-center text-zinc-500">
                            {{ trim($search) !== '' ? 'Nenhum resultado para esta pesquisa.' : 'Nenhum setor cadastrado.' }}
                        </x-modern.table.cell>
                    </x-modern.table.row>
                @endforelse
            </x-modern.table.rows>
    ...
    <x-modern.modal name="delete-record" wire:model.self="showDelete" title="Confirmar exclusão" class="w-full max-w-md">
        <form wire:submit="delete" class="space-y-5">
            <p class="text-sm leading-6">Deseja excluir o setor <strong>{{ $deleteName }}</strong>?</p>
            <p class="text-sm text-zinc-500 dark:text-zinc-400">Os tickets e usuários serão preservados e ficarão sem este setor. As categorias perderão o vínculo com ele.</p>
            @error('delete')
