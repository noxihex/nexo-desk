<div class="space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <x-modern.button :href="route('categorias.create')" variant="filled" color="green" icon="plus">Nova categoria</x-modern.button>
    </div>
    ...
    <x-modern.card class="overflow-hidden">
        <x-modern.table :paginator="$records">
            <x-modern.table.columns>
                ...
                <x-modern.table.column>Setores</x-modern.table.column>
            ...
            <x-modern.table.rows>
                @forelse($records as $record)
                    <x-modern.table.row :key="$record->id">
                        ...
                        <x-modern.table.cell class="max-w-sm whitespace-normal">{{ $record->setores->pluck('nome')->implode(', ') ?: 'Sem setor' }}</x-modern.table.cell>
                        <x-modern.table.cell>
                            <div class="flex flex-wrap gap-2">
                                <x-modern.button :href="route('categorias.edit', $record)" variant="filled" color="amber" size="sm" icon="pencil-square" :aria-label="'Editar '.$record->nome">Editar</x-modern.button>
                                <x-modern.button variant="filled" color="red" size="sm" icon="trash" wire:click="confirmDelete({{ $record->id }})" :aria-label="'Excluir '.$record->nome">Excluir</x-modern.button>
                            </div>
                        </x-modern.table.cell>
                    </x-modern.table.row>
                @empty
                    <x-modern.table.row>
                        <x-modern.table.cell colspan="3" class="py-10 text-center text-zinc-500">
                            {{ trim($search) !== '' ? 'Nenhum resultado para esta pesquisa.' : 'Nenhuma categoria cadastrada.' }}
                        </x-modern.table.cell>
                    </x-modern.table.row>
                @endforelse
            </x-modern.table.rows>
    ...
    <x-modern.modal name="delete-record" wire:model.self="showDelete" title="Confirmar exclusão" class="w-full max-w-md">
        <form wire:submit="delete" class="space-y-5">
            <p class="text-sm leading-6">Deseja excluir a categoria <strong>{{ $deleteName }}</strong>?</p>
            <p class="text-sm text-zinc-500 dark:text-zinc-400">Os tickets serão preservados e ficarão sem esta categoria.</p>
            @error('delete')
